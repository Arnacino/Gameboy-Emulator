...
TEST_CASE_METHOD(TestFixture, "Add Instructions", "[instructions]") {
    SECTION("ADD A,B") {
        registers.a = 0x3A;
        registers.b = 0xC6;
        instructions.addAR(&registers.b);
...
TEST_CASE_METHOD(TestFixture, "Arithmetic Instructions", "[instructions]") {
    SECTION("ADC A,B without carry") {
        ...
        instructions.adcAR(&registers.b);
    ...
    SECTION("SUB A,B") {
        registers.a = 0x42;
        registers.b = 0x3A;
        instructions.subAR(&registers.b);
...
TEST_CASE_METHOD(TestFixture, "Control Flow Instructions", "[instructions]") {
    ...
    SECTION("RET C") {
        ...
        REQUIRE(registers.sp == 0xFFFC);  // SP non dovrebbe cambiare
    }
...
//MANCA MEMORY READ WRITE + NON ANCORA IMPLEMENTATE
